...
public class SchemaOnePointFourStrategy extends SchemaCommonStrategy {
    ...
    protected PublicationList getPublicationList(Submission submissionSummary) {
        ...
        if(list.getPublication().isEmpty()) {
            noPublication(list);
        }
        return list;
    }

    private static void noPublication(PublicationList list) {
    ...
    protected DatasetFileList createDatasetFileList(Submission submissionSummary, String datasetPathFragment) {
        ...
        for (DataFile dataFile : submissionSummary.getDataFiles()) {
            ...
            switch (dataFile.getFileType()) {
                case RAW:
                    fileParam = createCvParam("MS:1002846", fileUri, "Associated raw file URI", MS_CV);
                    ...
                    break;
                case RESULT:
                    fileParam = createCvParam("MS:1002848", fileUri, "Result file URI", MS_CV);
                    break;
                case SEARCH:
                    fileParam = createCvParam("MS:1002849", fileUri, "Search engine output file URI", MS_CV);
                    break;
                case PEAK:
                    fileParam = createCvParam("MS:1002850", fileUri, "Peak list file URI", MS_CV);
                    break;
                case GEL:
                    fileParam = createCvParam("MS:1002860", fileUri, "Gel image file URI", MS_CV);
                    break;
                case OTHER:
                    fileParam = createCvParam("MS:1002851", fileUri, "Other type file URI", MS_CV);
                    break;
                default:
                    fileParam = createCvParam("MS:1002845", fileUri, "Associated file URI", MS_CV);
                    break;
            }
